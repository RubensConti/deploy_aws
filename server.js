...
app.get('/home',  async (req, res) => {
  ...
  //console.log( req.session.views.name)
  ...
  const banco = await Gasto.find({ usuario: req.session.views._id}).sort({data:1})
  const ent = await Entrada.find({ usuario: req.session.views._id}).sort({data:1})
...
let mes_G = await Gasto.find({ usuario: req.session.views._id}).sort({data:1})
...
let mes_E = await Entrada.find({ usuario: req.session.views._id}).sort({data:1})
...
for(let i=0;i<12;i++){
  ...
  const ultimos = await Gasto.find({ usuario: req.session.views._id}).sort({data:1}).limit(8)
  ...
  res.render(__dirname + '/static/index.html', {gastos: valor,num:quant, entradas:valor_e,unidades:quant_e, dadosG:dados_g, dadosE:dados_e,nome:req.session.views.name})
}) 
...
app.get('/profile', async(req, res) => {
  ...
  const banco = await User.findOne({ '_id': req.session.views._id})
...
app.get('/dividas', async (req, res) => {
  ...
  const banco = await Divida.find({ usuario: req.session.views._id}).sort({numero:1})
...
app.get('/devendo', async (req, res) => {
  ...
  const banco = await Divida.find({ devedor: req.session.views.email}).sort({numero:1})
...
app.get('/mes', async (req, res) => {
  ...
  let dados = []
  const banco = await Gasto.find({ usuario: req.session.views._id , data : {$gte: t.toLocaleDateString('en-CA') }}).sort({data:1})
...
app.get('/entradaMes', async (req, res) => {
  ...
  let dados = []
  const banco = await Entrada.find({ usuario: req.session.views._id , data : {$gte: t.toLocaleDateString('en-CA') }}).sort({data:1})
...
app.get('/totalE', async (req, res) => {
  ...
  let dados = []
  const banco = await Entrada.find({ usuario: req.session.views._id }).sort({data:1})
...
app.get('/metas',async (req, res) => {
  ...
  const banco = await Meta.find({ usuario: req.session.views._id}).sort({numero:1})
...
app.post('/cadastroM', async (req, res) => {
  ...
  let x = 1
  const banco = await Meta.findOne({ usuario: req.session.views._id}).sort({numero: -1})
...
app.post('/', async (req, res) => {
     ...
     let senhaExist = await User.findOne({ password: hashedSenha})
     
     if(!senhaExist){ 
       return res.status(442).json(res.redirect('/'))       
     }
 
    //req.session.views= senhaExist
...
app.post('/form', async (req, res) => {
  ...
  try{          
     User.collection.replaceOne({ "_id":req.session.views._id } , {"name": req.session.views.name,
      "email": req.session.views.email,
      "password": req.session.views.password,
...
app.post('/metas', async (req, res) => {
  ...
  app.post('/profile', async (req, res) => {
    ...
    User.collection.updateOne({'_id': req.session.views._id},{$set:{'emprego': req.body.emprego, 'email': req.body.email,
  ...
  app.post('/dividas', async (req, res) => {
    ...
    let x = 1
    const banco = await Divida.findOne({ usuario: req.session.views._id}).sort({numero: -1})
